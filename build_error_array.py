...
Explanation: 
    ...
    Therefore we can build a nx4x2 array, where 
        - There are n rows of 4x2 arrays
            - 4 is the number of lists (representing the different states)
                - 2 is the number of numbers in the lists 
    ...
    for row in range(n_output_bits):
        ...
        for col in range(number_of_states):
            ...
            for input in input_bits:
                ...
                error = comm.utilities.hamming_dist(np.array(curr_bits), possible_output_bits)
